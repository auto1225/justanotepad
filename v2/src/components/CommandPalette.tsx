...
export function CommandPalette(p: CommandPaletteProps) {
  ...
  const commands: Command[] = useMemo(() => {
    ...
    return [
      ...
      { id:'sidebar', cat:'보기', icon:'list-bullet', label:'사이드바 켬/끔', desc:'메모 목록 열기/접기.', run: toggleSidebar },
      { id:'layout-print', cat:'보기', icon:'page', label:'인쇄 모양 (인쇄 레이아웃)', desc:'A4/A3/B4 종이와 여백을 실제 페이지처럼 표시.', run: () => ui.setViewLayout('print') },
      { id:'layout-draft', cat:'보기', icon:'file-text', label:'초안 모양 (초안 레이아웃)', desc:'페이지 경계 없이 긴 문서를 빠르게 편집.', run: () => ui.setViewLayout('draft') },
      ...
      { id:'zoom-in', cat:'보기', icon:'zoom-in', label:'줌 크게', desc:'본문 +10%.', hint:'Ctrl+=', run: zoomIn },
      { id:'zoom-out', cat:'보기', icon:'zoom-out', label:'줌 작게', desc:'본문 -10%.', hint:'Ctrl+-', run: zoomOut },
      { id:'zoom-reset', cat:'보기', icon:'refresh-cw', label:'줌 100% 로', desc:'기본 크기로.', hint:'Ctrl+0', run: zoomReset },
      ...
      { id:'h-num', cat:'보기', icon:'hash', label:'제목 번호 켬/끔', desc:'1, 1.1, 1.1.1 자동 번호.', run: toggleHeadingNumbers },
      ...
      toggleSidebar, toggleHeadingNumbers, toggleReading, toggleSpellCheck, toggleRulers, theme, setTheme, p, ui])
