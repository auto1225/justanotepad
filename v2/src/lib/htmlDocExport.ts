...
export function downloadHtmlFile(bodyHtml: string, title: string): void {
  const html = `<!doctype html><html lang="ko"><head><meta charset="utf-8"><title>${title.replace(/</g, '&lt;')}</title><style>${DOC_EXPORT_CSS}</style></head><body>${bodyHtml}</body></html>`
...
export function downloadDocFile(bodyHtml: string, title: string): void {
  const html = `<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40"><head><meta charset="utf-8"><title>${title.replace(/</g, '&lt;')}</title><style>${DOC_EXPORT_CSS}</style></head><body>${bodyHtml}</body></html>`
