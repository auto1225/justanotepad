...
export async function writeFromTranscript(
  ...
  const prompt = [
    ...
    /* 받아 적은 글 앞의 [12:34] 는 녹음의 그 대목이다. 그것을 버리면 노트에서 소리로 돌아갈 길이 끊긴다. */
    '- 받아 적은 글 앞에 [12:34] 처럼 시각이 붙어 있으면, 중요한 대목의 절 제목 끝이나 문장 끝에',
    '  그 시각을 [12:34] 그대로 남긴다. 나중에 녹음의 그 자리로 돌아갈 실마리다.',
    '  모든 문장에 붙이지 않는다 — 절마다 한둘이면 넉넉하다.',
    '',
    '지킬 것:',
    QUALITY_RULE,
    '',
