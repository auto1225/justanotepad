...
test.describe('되돌리기는 글을 잃지 않는다', () => {
  ...
  /**
   * 칸 **속** 표의 칸에 친 글도 되돌릴 수 있어야 한다.
   *
   * 바깥 표 한 행이 한 쪽보다 길면 그 행을 두 행으로 가르는 「깊이 나눔」 이 돈다
   * (tableSplit.ts 의 splitTableDeepAcrossPages). 그 걸음이 바깥 표를 통째로 지우고 새 표
   * 둘을 넣던 시절에는 표 안의 자리가 매핑에서 모두 사라졌다 — 우리가 쪽 경계·표 나눔에서
   * 없앤 바로 그 고장이 거기 하나 남아 있었다.
   * 실측: 안쪽 칸에 50자를 치고 Ctrl+Z 를 세 번 눌러도 468자 → 438자에서 멎고 「속칸에친글」 이 남았다.
   *
   * 이제는 안쪽 표의 행과 행 사이를 깊이 4 로 **쪼개기만** 한다 (안쪽표·칸·행·바깥표를
   * 닫았다 연다). 끼어드는 것은 구조 표시 여덟 글자뿐이라 칸 속 자리가 모두 살아남는다.
   */
  test('칸 속 표의 칸에 친 글도 되돌릴 수 있다', async ({ page }) => {
    test.setTimeout(240000)
    await 새메모(page)
    await 붙여넣기(page,
      '<p>표 앞</p><table><tbody>' +
